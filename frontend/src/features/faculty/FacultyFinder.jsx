  ...
  >
    <div className="flex items-start gap-4">
      ...
      <div className="min-w-0 flex-1">
        <p
          className="font-semibold text-white truncate group-hover:text-blue-300 transition-colors"
          style={{ fontFamily: "Sora, sans-serif" }}
        >
          {faculty.name}
        </p>
        <p className="text-sm text-blue-400/80 mt-0.5">{faculty.designation}</p>
        <span className="inline-block mt-1.5 text-xs px-2 py-0.5 rounded-full bg-white/5 text-white/40 border border-white/8">
          {faculty.department}
        </span>
        <div className="flex items-center gap-1.5 mt-3 text-xs text-green-400/80">
          <MapPin className="h-3 w-3 shrink-0" />
          <span className="truncate">
            {faculty.cabin} · {faculty.block}
          </span>
        </div>
      </div>
    </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <a
              ...
              className="flex items-center gap-2.5 bg-white/4 hover:bg-white/8 border border-white/8 rounded-xl p-3.5 transition-colors"
            >
              <Phone className="h-4 w-4 text-blue-400 shrink-0" />
              <div className="min-w-0">
                <p className="text-xs text-white/30">Phone</p>
                <p className="text-sm text-white truncate">{faculty.phone}</p>
              </div>
            </a>
            <a
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm text-white truncate">{faculty.email}</p>
              </div>
            </a>
          </div>
          ...
          <div className="flex items-center gap-2.5 bg-white/4 border border-white/8 rounded-xl p-3.5">
            ...
            <div>
              <p className="text-xs text-white/30">Availability</p>
              <p className="text-sm text-white">{faculty.availability}</p>
            </div>
          </div>
          ...
          <div className="bg-white/4 border border-white/8 rounded-xl p-3.5">
            ...
            <div className="flex flex-wrap gap-2">
              {faculty.subjects.map((s) => (
                <span
                  key={s}
                  className="bg-purple-500/10 text-purple-300 text-xs px-2.5 py-1 rounded-full border border-purple-500/20"
                >
                  {s}
                </span>
              ))}
  ...
  return (
    <div className="min-h-screen bg-[#060d1f] dot-pattern">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-white/30 text-xs mt-0.5">
              Find faculty cabins & contact info
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
        {isLoading && (
          <p className="text-white/40 text-sm mb-4">Loading faculty data...</p>
        )}
