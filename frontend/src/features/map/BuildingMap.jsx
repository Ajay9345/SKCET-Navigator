...
const MapComponent = ({ apiKey }) => {
  ...
  const [buildingProfiles, setBuildingProfiles] = useState({});
  ...
  const buildings = useMemo(
    () => Object.keys(locations).map((name) => ({ name })),
    [locations],
  );
  ...
  useEffect(() => {
    const fetchMapData = async () => {
      try {
        const [locationsRes, photosRes, profilesRes] = await Promise.all([
          fetch("/api/data/locations"),
          fetch("/api/data/photos"),
          fetch("/api/data/profiles"),
        ]);

        const [locationsData, photosData, profilesData] = await Promise.all([
          locationsRes.json(),
          photosRes.json(),
          profilesRes.json(),
        ]);
        ...
        setBuildingPhotos(photosData || {});
        setBuildingProfiles(profilesData || {});
      } catch (error) {
        console.error("Error fetching building data:", error);
        setMapError("Error loading building data");
      } finally {
  ...
  return (
    <>
                  ...
                  >
                    <div>
                      ...
                      <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-2">
                        ...
                        >
                          <strong className="truncate text-primary/80">
                            Status:
                          </strong>
                          <span
                            className={cn(
                              "truncate max-w-[60%] text-ellipsis overflow-hidden",
                              getBuildingState(building.name) === "open"
                                ? "text-green-500"
                                : "text-red-500",
                            )}
                          >
                      ...
                      <div className="mt-3 flex gap-2">
                        ...
                        {buildingPhotos[building.name] && (
                          <Button
                            ...
                            onClick={() => setGalleryBuilding(building)}
                          >
                            <Images className="mr-2 h-4 w-4" />
                            Photos ({buildingPhotos[building.name].length})
                          </Button>
                        )}
                        {buildingProfiles[building.name] && (
                          <Button
                            variant="outline"
                            className="flex-1 border-primary/30 text-primary hover:bg-primary/10"
                            onClick={() => setProfileBuilding(building)}
                          >
          ...
          <Card className="w-full max-w-2xl h-[600px] overflow-hidden shadow-lg">
            <CardContent className="p-0 h-full">
              ...
              >
                ...
                {buildings
                  .filter((building) => locations[building.name])
                  .map((building) => (
                    <Marker
                      key={building.name}
                  ...
                  >
                    <div className="p-2">
                      ...
                      <p
                        className={cn(
                          "text-sm font-bold",
                          getBuildingState(selectedBuilding.name) === "open"
                            ? "text-green-500"
                            : "text-red-500",
                        )}
                      >
      ...
      {galleryBuilding && buildingPhotos[galleryBuilding.name] && (
        <BuildingGallery
          buildingName={galleryBuilding.name}
          photos={buildingPhotos[galleryBuilding.name]}
          onClose={() => setGalleryBuilding(null)}
        />
      )}
      {profileBuilding && buildingProfiles[profileBuilding.name] && (
        <BuildingProfileModal
          buildingName={profileBuilding.name}
          profile={buildingProfiles[profileBuilding.name]}
  ...
  if (!apiKey) {
    return (
      <div className="flex h-screen items-center justify-center bg-[#060d1f]">
        ...
        <span className="ml-2 text-white">Loading Map Configuration...</span>
      </div>
    );
  }
...
export default BuildingMapWrapper;
